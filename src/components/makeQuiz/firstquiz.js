...
class Firstquiz extends React.Component {
  ...
  componentDidMount = () => {
    ...
    db.collection("QG3-Quiz")
      .doc(this.props.match.params.quiz_id)
      .get()
      .then(function(doc) {
        if (doc.exists) {
          dat.push(doc.data());
  ...
  updatastate = () => {
    ...
    if (end.length) {
      this.setState({ loaded: false, finnished: true });
      return;
    }
    ...
    this.setState({
      Answers: ans.map(answer =>
        answer.map(ans => (
          <Answer lagra={this.lagrasvar} text={ans} value={false} />
        ))
      )
    });
    ...
    this.setState({
      Correctans: <Answer lagra={this.lagrasvar} text={correct} value={true} />
    });
